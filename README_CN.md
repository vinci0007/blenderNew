...
**[更新日志](CHANGELOG_CN.md)** | **[Changelog](CHANGELOG.md)** | **[Release Notes](RELEASE_NOTES.md)**
...
- 新增 Blender executor 健康状态与自动恢复能力，可在队列卡住且没有技能正在执行时重挂 Blender app timer。
- 新增客户端执行前 backpressure，避免超时或 resume 后继续向异常队列堆积 `execute_skill` 请求。
- 新增未开始执行的 queued job deadline，客户端断开或排队过期后不会再晚到修改场景。
- 新增 `--image` / `--image-file` 多模态规划输入，可把 prompt 与一张或多张参考图一起发送给支持视觉输入的 LLM。
- 调整 adaptive batch quality repair：材质、灯光、动画、相机、渲染等后续阶段缺口会作为 pending work 继续推进，不再反复困在当前几何 batch。
- 新增修复计划保护，避免 batch cleanup 删除后续步骤仍引用的父对象或控制对象。
- Blender 5.1 API 文档兼容测试现在支持读取本地 zip，且在仓库未上传 reference 文档时会自动 skip。
- 更新为自定义非商业许可证：允许个人学习、研究、评估和个人非商业二次开发；个人衍生项目必须开源，并保留原作者、仓库与 LICENSE。
...
- 调整需求评估为 `auto` 模式下 LLM 优先；本地简单模型判断只作为辅助证据，不再覆盖用户明确的后续阶段要求。
- 明确 `uv_texture_material` 阶段包含 UV、贴图、简单颜色分配、材质分配和 PBR/material preset。
- 新增 Analyzer 近似合法 JSON 的保守修复，减少 LLM 少闭合符时的解析兜底噪音。
- 减少反馈 Analyzer 调用频率，将小阶段切换归并到主要/adaptive 分析阶段。
...
可以多次传入 `--image`，为同一个建模请求提供多张视觉参考图。图片会以内嵌 data URL 形式转发给当前配置的 LLM 协议；实际效果取决于所选模型/服务商是否支持视觉输入。
...
1. 先评估用户真正想交付到哪个阶段；当 LLM 可用时优先采用 LLM 的阶段判断。
2. 选择几何、UV/材质、灯光、动画、渲染等阶段。
3. 本地规则只作为辅助证据和冲突检查；严重冲突交给第二次 LLM 判断。
4. 按阶段所需能力压缩技能集，而不是简单做 top-k 裁剪。
5. 请求 LLM 生成结构化计划。
6. 按技能 Schema 做归一化和参数校验。
7. 执行时结合反馈捕获、局部重规划、任务恢复和任务级日志继续推进。

这条链路主要用来避免几类常见问题：

- 纯几何任务被误判成材质或渲染任务
- 简单资产请求中明确的颜色、材质、灯光、动画或渲染要求被本地规则吞掉
- 上下文压缩时把关键建模技能裁掉
...
可以在 `config.toml` 中通过 `base_url`、`api_protocol`、`chat_completions_path`、`responses_path`、代理相关开关、额外请求头、鉴权模式和能力探测等配置做细化适配。

对 Anthropic Messages 兼容网关，要把请求协议和鉴权方式分开配置。例如 LongCat 的 Anthropic 兼容接口使用 Messages 请求体，但鉴权需要 Bearer：
...
`auth_scheme = "auto"` 使用默认策略；`auth_scheme = "bearer"` 会发送 `Authorization: Bearer <api_key>`；`auth_scheme = "x-api-key"` 用于 Anthropic 原生风格的 API-key 请求头。

## Blender Executor 健康状态

技能执行会进入 Blender 队列，并由 Blender 主线程 app timer 消费。VBF 现在通过 `vbf.executor_status` 暴露 executor 健康状态，并可在安全条件下恢复停止轮询的 app timer。

保守恢复规则：

- 如果存在 `current_job`，说明有技能正在执行，VBF 会等待，不会打断它。
- 如果 `queue_size > 0`、`current_job` 为空且 `last_poll_age_s` 已过期，VBF 可以重挂 app timer。
- 客户端断开或排队过期的未开始任务会被跳过，避免旧请求晚到修改场景。
...
`tests/test_blender_51_api_docs_compat.py` 会在本地存在 Blender 5.1 Python API 文档时校验 `bpy.ops` 调用。可以本地保留已解压文档或 `reference/blender_python_reference_5_1.zip`；仓库不要求上传 reference 文档，缺失时该测试会自动 skip。
...
- 面向用户的版本发布摘要放在 [RELEASE_NOTES.md](RELEASE_NOTES.md)。
- `pyproject.toml` 的 `[project].version` 是唯一版本来源，使用数字三段式 `x.x.x` 格式。
- 每一段数字都可以是多位数，例如 `2.13.105`。
- 自动触发的 GitHub Release 只发布第一段或第二段递增、且第三段为 `0` 的大版本/阶段版本，例如 `v1.0.0` 或 `v1.1.0`。
- 手动触发的 GitHub Release workflow 可以发布任意数字三段式项目版本，包括 `v1.1.1` 这类补丁版本。
- 第三段大于 `0` 的补丁版本，例如 `v1.1.1`，默认只记录在 changelog 中，除非手动触发发布。
- 准备新的 `x.y.0` 发布版本时，需要同步更新 `pyproject.toml` 和 `RELEASE_NOTES.md` 中对应版本段落。
...
| 自检显示运行中但任务仍卡住 | 检查 `vbf.executor_status`；如果 `queue_size > 0`、`current_job=null` 且 `last_poll_age_s` 很高，更新/重载插件，让 executor 自动恢复重挂 app timer |
| LLM 配置未生效 | 使用 `vbf/config/config.toml`，不要再用旧 JSON 配置 |
| LongCat 在 `/anthropic/v1/messages` 返回 `401 missing_api_key` | 保持 `api_protocol = "claude_responses"`，并设置 `auth_scheme = "bearer"`，让 VBF 发送 `Authorization: Bearer <api_key>` |
| 任务超时后 Blender 稍后又修改了场景 | 更新/重载插件；未开始执行的旧队列任务现在带有 deadline，客户端断开后会被跳过 |
| 新任务总提到旧任务对象 | 保持 `[project.scene].task_scene_policy = "isolate"`，默认只把当前任务相关对象送入规划/反馈上下文 |
| 简单请求丢失明确的颜色/材质/渲染意图 | 需求评估现在优先采用 LLM 判断；`llm.requirement_assessment.prefer_geometry_for_simple_model_requests = true` 只作为辅助证据，不再硬覆盖 LLM |
| Analyzer 出现 `LLM parse error` | 先看 `vbf/cache/last_gen_fail.txt`；新版本会修复简单缺失闭合符，并尽量从失败内容中恢复 `quality/reason/score/suggestions` |
...
Vibe-Blender-Flow 使用自定义非商业许可证。允许个人学习、研究、评估和个人非商业二次开发；未经单独授权，不允许商业使用或企业商业使用。任何个人修改、个人二次开发或个人衍生项目必须开源，并保留原作者、原仓库和 LICENSE。详见 [LICENSE](LICENSE)。
