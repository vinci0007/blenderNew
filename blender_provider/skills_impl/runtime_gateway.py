...
def ops_list(prefix: str | None = None, head_limit: int = 0) -> Dict[str, Any]:
    """
    Enumerate available bpy.ops operators, returned as ['mesh.primitive_cube_add', ...].
    """
    out: List[str] = []
...
def ops_invoke(
    ...
    """
    Invoke bpy.ops operator by id string.
    """
...
def ops_introspect(operator_id: str) -> Dict[str, Any]:
    """
    Inspect bpy.ops operator RNA metadata for parameter guidance.
    """
    op = _resolve_operator(operator_id)
    # access operator RNA in Blender
    rna = op.get_rna_type()
    props = []
...
def ops_search(query: str, head_limit: int = 20) -> Dict[str, Any]:
    """
    Search bpy.ops operator ids by keyword with simple scoring.
    """
...
def types_list(prefix: str | None = None, head_limit: int = 0) -> Dict[str, Any]:
    """
    Enumerate bpy.types names (classes/types exposed by Blender).
    """
    out: List[str] = []
...
def data_collections_list(prefix: str | None = None, head_limit: int = 0) -> Dict[str, Any]:
    """
    Enumerate top-level collections/attributes on bpy.data (e.g. objects, materials, meshes).
    """
    out: List[str] = []
