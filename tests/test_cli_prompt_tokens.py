...
def test_parser_accepts_split_prompt_tokens_and_other_flags():
    ...
    prompt, resume_arg, visual_inputs = _resolve_prompt_and_resume_arg(parser, args)
...
def test_prompt_can_attach_image_inputs(tmp_path):
    parser = _build_parser()
    image_file = tmp_path / "reference.png"
    image_file.write_bytes(b"\x89PNG\r\n\x1a\n")

    args = parser.parse_args(["--prompt", "create", "this", "--image", str(image_file)])
    prompt, resume_arg, visual_inputs = _resolve_prompt_and_resume_arg(parser, args)

    assert prompt == "create this"
    assert resume_arg == f'--prompt "create this" --image "{image_file}"'
    assert len(visual_inputs) == 1
    assert visual_inputs[0]["type"] == "image_url"
    assert visual_inputs[0]["image_url"]["url"].startswith("data:image/png;base64,")
...
@pytest.mark.asyncio
async def test_cli_run_attaches_visual_inputs(monkeypatch, tmp_path, capsys):
    captured = {}
    ...
    class _FakeClient:
        ...
        def set_visual_inputs(self, visual_inputs):
            captured["visual_inputs"] = visual_inputs
    ...
    visual_inputs = [{"type": "image_url", "image_url": {"url": "data:image/png;base64,AAAA"}}]
    monkeypatch.setattr("vbf.app.cli.load_project_paths", lambda: {"logs_dir": str(tmp_path)})
    monkeypatch.setattr("vbf.app.cli.VBFClient", _FakeClient)

    exit_code = await _run(
        prompt="model reference",
        resume_prompt_arg='--prompt "model reference" --image "ref.png"',
        host=None,
        port=None,
        blender_path=None,
        resume=None,
    ...
    out = capsys.readouterr().out
    assert exit_code == 0
    assert captured["visual_inputs"] == visual_inputs
    assert "[VBF] Visual inputs attached: 1 image(s)" in out
